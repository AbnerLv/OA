...
import com.lzb.oa.setting.SettingMap;
...
public class TaskDetail extends BaseActivity implements OnClickListener {
     ...
     * @param roomerRent
     *            出租或出售
     * @param roomerComplete
     *            交易是否完成
     * @param roomerEmpNo
     *            跟进人
     * @param roomerHouseCity
     *            房子所在城市
     * @param roomerHouseAddress
     *            房子详细地址
     */
    public static void startTaskDetail(Context context, String roomerNo,
            String roomerName, String roometSex, String roomerPhoneNo,
            String roomerHouseNo, String roomerDate, String roomerPeriod,
            String roomerRent, String roomerComplete, String roomerEmpNo,
            String roomerHouseCity, String roomerHouseAddress) {
    ...
    private void init() {
        ...
        if ("".equals(intent.getStringExtra("roomerEmpNo"))) {
            ...
            if (Integer.parseInt(intent.getStringExtra("roomerComplete")
                    .toString()) == 0) {
                        ...
                        public void onResponse(JSONObject response) {
                            try {
                                if (Integer.parseInt(response
                                        .getString("success")) == 1) {
                                    Toast.makeText(TaskDetail.this, "任务已取消",
                    ...
                    TASK_DETAIL_URL, json, new Listener<JSONObject>() {
                        ...
                        public void onResponse(JSONObject response) {
                            try {
                                if (Integer.parseInt(response
                                        .getString("success")) == 1) {
                                    Toast.makeText(TaskDetail.this, "你已成功领取任务",
                                            Toast.LENGTH_SHORT).show();
                                    tvTaskDetailComplete
                        ...
                        @Override
                        public void onErrorResponse(VolleyError error) {
                            Toast.makeText(TaskDetail.this, "网络连接失败",
                                    Toast.LENGTH_SHORT).show();
                        }
                    });
