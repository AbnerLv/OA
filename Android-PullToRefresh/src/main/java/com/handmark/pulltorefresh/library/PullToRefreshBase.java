...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
    ...
    @Override public final boolean isPullToRefreshOverScrollEnabled() {
        return VERSION.SDK_INT >= VERSION_CODES.GINGERBREAD
                && mOverScrollEnabled && OverscrollHelper
    ...
    @Override public final void setOnRefreshListener(
    ...
    @Override public final void setPullToRefreshOverScrollEnabled(
            boolean enabled) {
    ...
    @Override protected final void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            mCurrentMode = Mode
                    .mapIntToValue(bundle.getInt(STATE_CURRENT_MODE, 0));
    ...
    @Override protected final void onSizeChanged(int w, int h, int oldw,
            int oldh) {
        if (DEBUG) {
    ...
    private void addRefreshableView(Context context, T refreshableView) {
        ...
        mRefreshableViewWrapper
                .addView(refreshableView, ViewGroup.LayoutParams.MATCH_PARENT,
    ...
    @SuppressWarnings("deprecation") private void init(Context context,
            AttributeSet attrs) {
        switch (getPullToRefreshScrollDirection()) {
        case HORIZONTAL:
        ...
        TypedArray a = context
                .obtainStyledAttributes(attrs, R.styleable.PullToRefresh);

        if (a.hasValue(R.styleable.PullToRefresh_ptrMode)) {
            mMode = Mode.mapIntToValue(
                    a.getInteger(R.styleable.PullToRefresh_ptrMode, 0));
        }

        if (a.hasValue(R.styleable.PullToRefresh_ptrAnimationStyle)) {
            mLoadingAnimationStyle = AnimationStyle.mapIntToValue(
                    a.getInteger(R.styleable.PullToRefresh_ptrAnimationStyle,
                            0));
        }
         ...
         */
        if (a.hasValue(
                R.styleable.PullToRefresh_ptrRefreshableViewBackground)) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrOverScroll)) {
            mOverScrollEnabled = a
                    .getBoolean(R.styleable.PullToRefresh_ptrOverScroll, true);
        }
    ...
    private void pullEvent() {
        ...
        case PULL_FROM_END:
            newScrollValue = Math
                    .round(Math.max(initialMotionValue - lastMotionValue, 0)
                            / FRICTION);
        ...
        default:
            newScrollValue = Math
                    .round(Math.min(initialMotionValue - lastMotionValue, 0)
                            / FRICTION);
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            if (mState != State.PULL_TO_REFRESH && itemDimension >= Math
                    .abs(newScrollValue)) {
        ...
        @Override public void run() {
            ...
            if (mStartTime == -1) {
                ...
                final int deltaY = Math
                        .round((mScrollFromY - mScrollToY) * mInterpolator
