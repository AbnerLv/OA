...
@TargetApi(9) public final class OverscrollHelper {
    ...
    public static void overScrollBy(final PullToRefreshBase<?> view,
            final int deltaX, final int scrollX, final int deltaY,
            final int scrollY, final int scrollRange,
    ...
    public static void overScrollBy(final PullToRefreshBase<?> view,
        ...
        if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
            ...
            // touch
            if (mode.permitsPullToRefresh() && !isTouchEvent
                    && deltaValue != 0) {
                final int newScrollValue = (deltaValue + scrollValue);

                if (PullToRefreshBase.DEBUG) {
                    Log.d(LOG_TAG,
                            "OverScroll. DeltaX: " + deltaX + ", ScrollX: "
                                    + scrollX + ", DeltaY: " + deltaY
                                    + ", ScrollY: " + scrollY + ", NewY: "
                                    + newScrollValue + ", ScrollRange: "
                                    + scrollRange + ", CurrentScroll: "
                                    + currentScrollValue);
                }
                ...
                if (newScrollValue < (0 - fuzzyThreshold)) {
                    ...
                    if (mode.showHeaderLoadingLayout()) {
                        ...
                        view.setHeaderScroll(
                                (int) (scaleFactor * (currentScrollValue
                                        + newScrollValue)));
                    }
                    ...
                    if (mode.showFooterLoadingLayout()) {
                        ...
                        || Math.abs(newScrollValue - scrollRange)
                        <= fuzzyThreshold) {
