...
import com.lzb.oa.R;
import com.lzb.oa.adapter.CompanyContactsAdapter;
...
public class ComplanyContacts extends BaseActivity {
    ...
    private final static String CONTACTS_URL = Constant.URL
            + "get_contacts.php";
    private final static String DEPARTMENT_URL = Constant.URL
            + "get_department.php";
    ...
    protected void onResume() {
        super.onResume();
        companyContactsAdapter = new CompanyContactsAdapter(this, father_array,
                son_array, elvCompanyContacts);
        getFatherArrry();
        getCompanyContacts();
    ...
    private void getFatherArrry() {
        ...
        // 向服务端发送请求
        JsonArrayRequest jsonArrayRequestFather = new JsonArrayRequest(
                DEPARTMENT_URL, new Listener<JSONArray>() {
                    ...
                    public void onResponse(JSONArray response) {
                        for (int i = 0; i < response.length(); i++) {
                            try {
                                ...
                                map.put("contactsDepartment",
                                        jsonObj.getString("emp_department"));
    ...
    private void getCompanyContacts() {
        ...
        JsonArrayRequest jsonArrayRequestSon = new JsonArrayRequest(
                CONTACTS_URL, new Listener<JSONArray>() {
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        for (int i = 0; i < response.length(); i++) {
                            try {
                                JSONObject jsonObj = response.getJSONObject(i);
                                HashMap<String, Object> map = new HashMap<String, Object>();
                                map.put("contactsChildName",
                                        jsonObj.getString("emp_name"));
                                map.put("contactsChildEmpNo",
                                        jsonObj.getString("emp_no"));
                                map.put("contactsChildPhoneNo",
                                        jsonObj.getString("emp_phone_no"));
                                map.put("contactsDepartment",
                                        jsonObj.getString("emp_department"));
                                son_array.add(map);
                            } catch (Exception e) {
